...
class UserManage(QDialog):
    ...
    def setUpUI(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['姓名', '密码'])
        # 不可编辑
    ...
    def getResult(self):
        ...
        # 数据库去掉了Name字段
        sql = "SELECT UserId, Password FROM User WHERE IsAdmin=0"
    ...
    def deleteUser(self):
        ...
        # Zgg
        # 归还所有书籍
        #sql = "SELECT * FROM User_Product  WHERE UserId='%s' AND BorrowState=1" % self.deleteId
        #self.query.exec_(sql)
        #timenow = time.strftime('%Y-%m-%d', time.localtime(time.time()))
        #updateQuery=QSqlQuery()
        #while (self.query.next()):
        #    ProductId=self.query.value(1)
        #    sql="UPDATE Product SET NumCanBorrow=NumCanBorrow+1 WHERE ProductId='%s'"% ProductId
        #    updateQuery.exec_(sql)
        #    self.db.commit()
        #sql="UPDATE User_Product SET ReturnTime='%s',BorrowState=0 WHERE UserId='%s' AND BorrowState=1"%(timenow,self.deleteId)
        #self.query.exec_(sql)
        #self.db.commit()
