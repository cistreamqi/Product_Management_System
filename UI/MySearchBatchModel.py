    ...
    def deleteKBM(self):
        ...
        for i, isSelected in enumerate(self.checkList):
            if isSelected == "Checked":
                ...
                # sql = "DELETE FROM T_Product WHERE ProductNO = '%s'" % (self.data_list[i][2])
                # query.exec(sql)
                sql = "DELETE FROM %s WHERE Num = '%s' AND Dataitem = '%s'" % (self.table, self.data_list[i][0], self.data_list[i][1])
                print(sql)
                query.exec(sql)
                # sql = "DELETE FROM T_Product_Component WHERE ProductID = '%s'" % (self.data_list[i][2])
                # query.exec(sql)
    ...
    def selectSingleTableForeign(self):
        ...
        for i, isSelected in enumerate(self.checkList):
            if isSelected == "Checked":
                ...
                alist=self.data_list[i]#czq 不知道为什么老出错，加了这个就还行
                query.exec(sql)
                break
        if query.next():
            alist = [str(query.value(i)) for i in range(self.tableForeignLength)]
    ...
    def selectSingleTable(self):
        ...
        for i, isSelected in enumerate(self.checkList):
            if isSelected == "Checked":
                ...
                break
        if query.next():
            alist = [str(query.value(i)) for i in range(self.tableLength)]
        # print(list)
        return alist
